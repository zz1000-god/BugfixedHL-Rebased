...
void V_CalcIntermissionRefdef(struct ref_params_s *pparams)
{
	...
	if (gEngfuncs.IsSpectateOnly())
	{
		...
		VectorCopy(CHudSpectator::Get()->m_cameraOrigin, pparams->vieworg);
		VectorCopy(CHudSpectator::Get()->m_cameraAngles, pparams->viewangles);
	}
...
void V_CalcNormalRefdef(struct ref_params_s *pparams)
{
	...
	float forward_offset = cl_viewmodel_ofs_forward.GetFloat();
	float right_offset = cl_viewmodel_ofs_right.GetFloat();
	float up_offset = cl_viewmodel_ofs_up.GetFloat();
...
void V_GetSingleTargetCam(cl_entity_t *ent1, float *angle, float *origin)
{
	...
	int flags = CHudSpectator::Get()->m_iObserverFlags;
...
void V_GetChasePos(int target, float *cl_angles, float *origin, float *angles)
{
	...
	if (CHudSpectator::Get()->m_autoDirector->value)
	{
		if (g_iUser3)
...
void V_GetMapFreePosition(float *cl_angles, float *origin, float *angles)
{
	...
	zScaledTarget[0] = CHudSpectator::Get()->m_mapOrigin[0];
	zScaledTarget[1] = CHudSpectator::Get()->m_mapOrigin[1];
	zScaledTarget[2] = CHudSpectator::Get()->m_mapOrigin[2] * ((90.0f - angles[0]) / 90.0f);

	AngleVectors(angles, forward, NULL, NULL);

	VectorNormalize(forward);

	VectorMA(zScaledTarget, -(4096.0f / CHudSpectator::Get()->m_mapZoom), forward, origin);
}
